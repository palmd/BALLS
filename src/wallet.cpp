...
// Snowballs: optional setting to unlock wallet for block minting only;
...
// Snowballs: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Snowballs: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // Snowballs: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // Snowballs: not avatar mode
                    {
...
// Snowballs: create coin stake transaction
...
// Snowballs: check 'spent' consistency between wallet and txindex
// Snowballs: fix wallet spent state according to txindex
...
// Snowballs: disable transaction (only for coinstake)
