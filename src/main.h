...
static const int64 MIN_TX_FEE = 640000000;
static const int64 MIN_RELAY_TX_FEE = 640000000;
...
static const int STAKE_START_TIME = 1414929600;
static const int STAKE_TARGET_SPACING = 60; // 1-minute block spacing 
...
static const int64 nMaxClockDrift = 64 * 60;        // two hours
...
public:
    ...
    {
        // Snowballs: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // Snowballs: get transaction coin age
...
public:
    ...
    // Snowballs: block signature - signed by coin base txout[0]'s owner
    ...
    // Snowballs: two types of block: proof-of-work or proof-of-stake
    ...
    // Snowballs: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // Snowballs: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // Snowballs: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // Snowballs: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // Snowballs: block index flags
    enum  
    {
